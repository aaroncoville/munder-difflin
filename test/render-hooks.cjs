 ...
 *
 * Why this exists: four defects have shipped past review in this repo because
 * every test covered an extracted pure function and NOTHING proved the
 * component still called it. Grepping the source for the call site is not a
 * test either — it goes green on a call that is present but wired to the wrong
 * state. So we run the actual component function, with the actual JSX, and
