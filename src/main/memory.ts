 ...
 *  inbox/outbox message JSON, a Codex worker's private CODEX_HOME, and the
 *  Claude-side `.claude/` (its ~17 harness-provided skills are identical in every
 *  agent dir, so mining them indexed the same SKILL.md boilerplate once per agent
 *  until it dominated the digest over the floor's real notes). `mempalace mine`
 *  honors .gitignore, so we drop one in each agent dir rather than touch the mine
 *  command.
 *
